...
const wineryOptions = computed(() => {
  const values = new Set<string>();
  for (const wine of props.wines ?? []) {
    const names = [wine?.wineryName, wine?.relatedLocale?.title, wine?.related_locale?.title];
...
const wineryLabel = computed(() => selectedWineryLocal.value || 'Tutte le cantine');
...
function applyFilters(
  ...
  return wines.filter((wine) => {
    ...
    if (wineryNeedle) {
      const wineryNames = [wine?.wineryName, wine?.relatedLocale?.title, wine?.related_locale?.title];
      const match = wineryNames.some((name) => norm(typeof name === 'string' ? name : null) === wineryNeedle);
      if (!match) {
        return false;
      }
    }
